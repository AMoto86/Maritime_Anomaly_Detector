...
SHIP_PREFIXES = ["PACIFIC", "ATLANTIC", "NORDIC", "ASIA",
                 "GLOBAL", "OCEAN", "STAR", "HORIZON"]

DESTINATIONS = ["SINGAPORE", "ROTTERDAM", "SHANGHAI", "HOUSTON",
...
def generate_sample_data(n_vessels=200, seed=42):
    ...
    for i in range(n_vessels):
        ...
        prefix = rng.choice(SHIP_PREFIXES)
        name = "MV {} {}".format(prefix, rng.integers(1, 99))
        ...
        if anomaly_roll < 0.05:
            ...
            # STS CLUSTER - bunched together, nearly stopped
            ...
            # AIS GAP - old timestamp to trigger gap detection
        ...
        records.append({
            "imo": imo, "mmsi": mmsi, "name": name,
            "vessel_type": vessel_type,
            "latitude": lat, "longitude": lon,
            "sog": sog, "cog": cog, "heading": heading, "rot": rot,
...
def generate_sample_data_with_gaps(n_vessels=200, seed=42):
    """Generate sample data where some MMSIs have multiple timestamps for gap detection."""
    rng = np.random.default_rng(seed)
    now = datetime.utcnow()
    records = []
    ...
    for mmsi in unique_mmsis:
        imo = str(rng.integers(9000000, 9999999))
        prefix = rng.choice(["PACIFIC", "ATLANTIC", "NORDIC"])
        name = "MV {} {}".format(prefix, rng.integers(1, 99))
        vessel_type = rng.choice(VESSEL_TYPES)
        lane_lat, lane_lon = rng.choice(SHIPPING_LANES)
